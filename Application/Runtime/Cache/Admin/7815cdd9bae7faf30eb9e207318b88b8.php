    ...
    <div class="add_content" id="form-article-add">
     <ul>
      ...
      <label class="label_name"><i>*</i>文章标题</label><span class="formControls col-10">
        <input type="text" id="form-field-1" class="col-xs-10 col-sm-5 " name="title" value="<?php echo ($data["title"]); ?>">
      </span>
      </li>
      <li class="clearfix Mandatory"><label class="label_name"><i>*</i>发布时间</label>
      <span class="formControls col-10">    
        <input type="text" id="start" class="form-control laydate-icon" name="time" value="<?php echo ($data["time"]); ?>" /> 
      </span>
      </li>
      <li class="clearfix"><label class="label_name"><i>*</i>所属分类</label>
       <span class="formControls col-4">
        <select class="form-control" id="classification" name="classification">
          ...
          <?php if($data["classification"] == 6 ): ?><option value="6" selected="selected">钢管知识</option>
            <option value="7">行业动态</option>
            <option value="17">热点资讯</option>
          <else if condition="$data.classification eq 7 " />
            <option value="6">钢管知识</option>
            <option value="7" selected="selected">行业动态</option>
            <option value="17">热点资讯</option>
            <?php else: ?>
            <option value="6">钢管知识</option>
            <option value="7">行业动态</option>
            <option value="17"  selected="selected">热点资讯</option><?php endif; ?>
       </select>
       </span>
      </li>
      <li class="clearfix"><label class="label_name">是否置顶</label>
        ...
        <span class="add_date l_f">
          <?php if($data["settop"] == 1 ): ?><label><input  type="radio" class="ace" name="settop" value="1" checked="checked"><span class="lbl">置顶</span></label>&nbsp;
         <label><input  type="radio" class="ace" name="settop" value="0" ><span class="lbl">不置顶</span></label>
          <?php else: ?>
            <label><input  type="radio" class="ace" name="settop" value="1" ><span class="lbl">置顶</span></label>&nbsp;
         <label><input  type="radio" class="ace" name="settop" value="0" checked="checked"><span class="lbl">不置顶</span></label><?php endif; ?>
     ...
     $(".Mandatory input[type$='text']").each(function(n){
          if($(this).val()=="")
          {
            ...
            console.log(data);
                ...
                $.ajax({
                url:"<?php echo U('Admin/Index/exitnewsCon');?>",//请求地址
                data:data,//发送的数据
                type:'POST',//请求的方式
